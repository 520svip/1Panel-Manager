...
// 容器分页列表（state: all | running | paused | created | restarting | removing | exited | dead）
export function searchContainers(panel, opts = {}) {
  return request(panel, 'POST', '/containers/search', {
    page: 1, pageSize: 200, order: 'null', orderBy: 'name', state: 'all',
    ...opts,
  });
}

// 容器批量操作（operation: up | start | stop | restart | kill | pause | unpause | remove）
export function operateContainers(panel, names, operation) {
  return request(panel, 'POST', '/containers/operate', { names, operation });
}

// Docker 服务状态
// V2 返回 { isExist, isActive } 对象；V1 返回字符串（如 "Running" / "Stopped" / "NotExist"），统一归一化为对象
// 注意：V1 的 "Stopped" 表示 Docker 已安装但服务停止，必须视为 isExist:true 且 isActive:false
export async function getDockerStatus(panel) {
  const r = await request(panel, 'GET', '/containers/docker/status');
  if (r.ok && typeof r.data === 'string') {
    const s = String(r.data).toLowerCase().trim();
    // 白名单：只有明确表示"未安装/不存在"的值才判为 isExist:false
    const NOT_EXIST = new Set(['', 'notexist', 'not exist', 'none', 'error', 'unknown']);
    r.data = {
      isExist: !NOT_EXIST.has(s),
      isActive: s.includes('run') || s === 'active' || s === 'healthy' || s === 'ok' || s === 'exist:true,active:true',
    };
  }
  return r;
}

// 容器实时统计（CPU/内存），V1/V2 均支持 /containers/list/stats，返回数组
export function listContainerStats(panel) {
  return request(panel, 'GET', '/containers/list/stats');
}

// Docker 服务操作（operation: start | restart | stop）
export function operateDocker(panel, operation) {
  return request(panel, 'POST', '/containers/docker/operate', { operation });
}

// 清理 Docker（pruneType: container | image | volume | network | buildcache）
export function pruneDocker(panel, pruneType) {
  return request(panel, 'POST', '/containers/prune', { pruneType });
}

// 镜像列表
export function listImages(panel) {
  return request(panel, 'GET', '/containers/image/all');
}

// 删除镜像（names 为镜像 ID 或 仓库:标签，force 忽略镜像占用）
export function removeImages(panel, names, force = true) {
  return request(panel, 'POST', '/containers/image/remove', { names, force });
}
...
// 可升级应用列表（V2 官方「检查更新」用法）
// update=true 只返回可更新的应用；sync=false 不触发远程商店同步
export function getUpdatableApps(panel) {
  return request(panel, 'POST', '/apps/installed/search', {
    page: 1, pageSize: 200, name: '', tags: [], update: true, sync: false,
  });
}

// 刷新应用商店对比（V1 检查更新前置步骤；V2 已改用 getUpdatableApps）
